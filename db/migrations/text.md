...
En index gjør søk og filtrering raskere, for eksempel ved statusfiltrering.

## Hva er en constraint?

En regel som begrenser verdier i kolonner eller tabeller for å sikre dataintegritet, for eksempel PRIMARY KEY, FOREIGN KEY, NOT NULL, UNIQUE og CHECK.

## Hva betyr dataintegritet?

At data er korrekte, konsistente og pålitelige, og at relasjoner mellom tabeller opprettholdes.

## Hva er en databasemigrasjon?

En strukturert måte å endre databasen på, som å legge til tabeller, kolonner eller constraints, for å holde den synkron med applikasjonen.
...
ENUM begrenser kolonnen til faste verdier, mens CHECK lar deg lage regler. ENUM er enklere for faste sett med verdier, CHECK er mer fleksibel.
...
- **email**: VARCHAR(255) NOT NULL UNIQUE — må fylles inn og være unik.
...
Id er primary key, og email er unik og ofte brukt i søk.
...
Det kan skape inkonsistens og feil i databasen.
